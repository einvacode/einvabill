...
<div class="lg:grid lg:grid-cols-[256px_minmax(0,1fr)] min-h-screen">
    ...
    <aside id="appSidebar" class="fixed inset-y-0 left-0 z-50 flex w-[256px] -translate-x-full flex-col bg-sidebar text-white transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0">
        <div class="relative flex shrink-0 flex-col items-center gap-2 border-b border-solid border-white/10 px-4 py-3">
            <?php if ($logo_src): ?>
                <?php // Company logos are wide banners (this one is 4,9:1), so the plate follows the
                      // logo instead of boxing it into a square. Stacked and centred, with the legal
                      // name underneath. ?>
                <span class="inline-flex max-w-full items-center justify-center rounded-md bg-white p-1">
                    <img src="<?= htmlspecialchars($logo_src) ?>" alt="<?= htmlspecialchars($company_name) ?>" class="h-7 w-auto max-w-[172px] object-contain">
                </span>
            <?php else: ?>
                <span class="grid h-9 w-9 shrink-0 place-items-center rounded-md bg-white/10"><i class="fas fa-wifi text-[15px]"></i></span>
            <?php endif; ?>
            <div class="max-w-full truncate text-center text-[12px] font-semibold leading-tight text-white/90"><?= htmlspecialchars($company_name) ?></div>
            <button type="button" class="absolute right-2 top-2 grid h-8 w-8 place-items-center rounded-md text-white/70 hover:bg-white/10 lg:hidden" onclick="closeSidebar()" aria-label="Tutup menu"><i class="fas fa-times"></i></button>
        </div>
    ...
    <div class="w-full max-w-lg p-3 pb-20" onclick="event.stopPropagation()">
        <div class="ui-card max-h-[80vh] overflow-y-auto p-4">
            ...
            <div class="grid grid-cols-3 gap-2">
                ...
                <a href="index.php?page=admin_receivables" class="sheet-item <?= $page == 'admin_receivables' ? 'active' : '' ?>"><i class="fas fa-hourglass-half"></i>Piutang</a>
