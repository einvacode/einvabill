...
switch ($page) {
    ...
    case 'admin_edit_quick_invoice':
        ...
        break;
    case 'admin_cash':
        require __DIR__ . '/views/admin/cash.php';
        break;
    case 'admin_expenses':
        ...
        if (!$invoice) {
            ...
            try {
                ...
                if ($cid > 0) {
                    ...
                    $arrears->execute([$cid, $inv_id, $tenant_id]);
                    $ar = $arrears->fetch();
            ...
            if (($invoice['status'] ?? '') === 'Lunas') {
                try {
                    ...
                    $pstmt->execute([$inv_id]);
                    $payment_info = $pstmt->fetch();
                } catch (Exception $e) {}
                // Build bulan_bayar label from invoice period
