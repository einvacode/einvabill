...
function finance_trend_svg(array $trend, string $id = 'trendChart'): string {
    ...
    for ($g = 0; $g <= 4; $g++) {
        ...
        $s .= '<line x1="' . $pad_l . '" x2="' . ($w - $pad_r) . '" y1="' . $gy . '" y2="' . $gy . '" stroke="var(--chart-grid)" stroke-width="1"/>';
        if ($g > 0) $s .= '<text x="' . $pad_l . '" y="' . ($gy - 4) . '" font-size="10" fill="var(--chart-label)">' . $short($max * $g / 4) . '</text>';
    }
    $base_y = $pad_t + $plot_h;
    foreach ($trend as $i => $m) {
        $cx = $pad_l + $group_w * $i + $group_w / 2;
        $last = $i === $n - 1;
        foreach ([['revenue', 'var(--chart-1)', 'Pendapatan', -1], ['expenses', 'var(--chart-2)', 'Pengeluaran', 1]] as [$key, $color, $name, $side]) {
            $v = (float)$m[$key];
            ...
            if ($last && $v > 0) $s .= '<text x="' . ($bx + $bar_w / 2) . '" y="' . ($top - 6) . '" font-size="10" font-weight="600" text-anchor="middle" fill="var(--chart-value)">' . $short($v) . '</text>';
            $s .= '</g>';
        }
        $s .= '<text x="' . $cx . '" y="' . ($h - 10) . '" font-size="11" text-anchor="middle" fill="var(--chart-label)">' . htmlspecialchars($m['label']) . '</text>';
    }
    $s .= '<line x1="' . $pad_l . '" x2="' . ($w - $pad_r) . '" y1="' . $base_y . '" y2="' . $base_y . '" stroke="var(--chart-axis)" stroke-width="1"/>';
    return $s . '</svg>';
}
...
function finance_rate_svg(array $trend, string $id = 'rateChart'): string {
    ...
    foreach ([0, 50, 100] as $g) {
        $s .= '<line x1="' . $pad_l . '" x2="' . ($w - $pad_r) . '" y1="' . $y($g) . '" y2="' . $y($g) . '" stroke="var(--chart-grid)"/>';
        $s .= '<text x="' . ($pad_l - 6) . '" y="' . ($y($g) + 3) . '" font-size="10" text-anchor="end" fill="var(--chart-label)">' . $g . '%</text>';
    }
    $pts = []; foreach ($trend as $i => $m) if ($m['rate'] !== null) $pts[] = [$x($i), $y($m['rate']), $m];
    if (count($pts) > 1) { $d = ''; foreach ($pts as $k => $p) $d .= ($k ? ' L' : 'M') . sprintf('%.1f %.1f', $p[0], $p[1]); $s .= '<path d="' . $d . '" fill="none" stroke="var(--chart-1)" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>'; }
    foreach ($pts as $k => $p) {
        $last = $k === count($pts) - 1;
        $s .= '<g tabindex="0"><circle cx="' . $p[0] . '" cy="' . $p[1] . '" r="5" fill="var(--chart-1)" stroke="var(--chart-dot-ring)" stroke-width="2"><title>' . htmlspecialchars($p[2]['label_long'] . ' · tingkat tagih ' . $p[2]['rate'] . '% (' . $p[2]['paid_n'] . '/' . $p[2]['billed_n'] . ' tagihan)') . '</title></circle>';
        if ($last) $s .= '<text x="' . $p[0] . '" y="' . ($p[1] - 10) . '" font-size="11" font-weight="600" text-anchor="middle" fill="var(--chart-value)">' . $p[2]['rate'] . '%</text>';
        $s .= '</g>';
    }
    foreach ($trend as $i => $m) $s .= '<text x="' . $x($i) . '" y="' . ($h - 10) . '" font-size="11" text-anchor="middle" fill="var(--chart-label)">' . htmlspecialchars($m['label']) . '</text>';
    return $s . '</svg>';
}
